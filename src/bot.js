...
const ANNOUNCE_CHANNEL_ID = '1481738819484127375';
const CV_REVIEW_CHANNEL_ID = '1516902031573516329';
const RECRUIT_CHANNEL_ID = '1481738853969694781';
const DISPO_CHANNEL_ID = '1481738981422137494';
// Pas de ROLE_ID nécessaire pour Harmony
// Système de vente de kits / paie
const SALES_CHANNEL_ID = '1481738813222027297';
...
const RECRUIT_ANNOUNCE_CHANNEL_ID = '1481738819484127375';
const GUILD_ID = '1481734303695114242';
// Rôles
const CITIZEN_ROLE_ID = '1481739161772884151';
// Pas de STAFF_ROLE_ID nécessaire pour Harmony
const ID_CARD_CHANNEL_ID = '1481738938761744395';
const DIRECTION_ROLE_ID = '1481739077303926845';
const COMMANDE_CATEGORY_ID = '1516902597733126334';
const CONTRAT_CATEGORY_ID = '1516902650208063630';
const TICKET_ANNOUNCE_CHANNEL_ID = '1481738813222027297'; // Channel pour l'annonce des tickets
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (interaction.commandName === 'kit') {
            try {
                ...
                const EMPLOYEE_CATEGORY_ID = '1481738695081197664';
        ...
        if (interaction.commandName === 'add') {
            try {
                ...
                const role1 = await interaction.guild.roles.fetch('1481739117967704186');
                const role2 = await interaction.guild.roles.fetch('1487296524936548434');
                const roleToRemove = await interaction.guild.roles.fetch('1481739134220501002');
        ...
        if (interaction.commandName === 'up') {
            try {
                ...
                const hasRoleME = targetMember.roles.cache.has('1481739106944811180');
                const hasRoleM = targetMember.roles.cache.has('1481739112049414335');

                let newNickname, newChannelName, gradeText;

                if (hasRoleME) {
                    // Promotion ME → CA
                    const roleToRemove = await interaction.guild.roles.fetch('1481739106944811180');
                    const roleToAdd = await interaction.guild.roles.fetch('1481739094412234773');
                    ...
                    // Promotion M → ME
                    const roleToRemove = await interaction.guild.roles.fetch('1481739112049414335');
                    const roleToAdd = await interaction.guild.roles.fetch('1481739106944811180');
                    ...
                    // Promotion AMT → M
                    const roleToRemove = await interaction.guild.roles.fetch('1481739117967704186');
                    const roleToAdd = await interaction.guild.roles.fetch('1481739112049414335');
                ...
                if (employeeChannel) {
                    ...
                    try {
                        if (gradeText === 'CA') {
                            // Pour CA: déplacer dans 1481738678488272927
                            await employeeChannel.setParent('1481738678488272927');
                            await employeeChannel.setPosition(0);
                        } else {
                            // Pour M et ME: déplacer dans les catégories 1481738689855094976 puis 1481738684243116123
                            // Première catégorie
                            await employeeChannel.setParent('1481738684243116123');
                            await employeeChannel.setPosition(0);
                            ...
                            // Deuxième catégorie
                            await employeeChannel.setParent('1481738689855094976');
                            await employeeChannel.setPosition(0);
                        }
                    } catch (moveError) {
        ...
        if (interaction.commandName === 'custom') {
            try {
                ...
                // Vérifier que la commande est utilisée dans la catégorie employés
                const EMPLOYEE_CATEGORY_ID = '1481738689855094976';
                if (interaction.channel.parentId !== EMPLOYEE_CATEGORY_ID) {
                    await interaction.reply({ content: '❌ Cette commande ne peut être utilisée que dans la catégorie "Employés".', ephemeral: true });
                    return;
                }
        ...
        if (interaction.commandName === 'remuneration') {
            try {
                ...
                const REGLEMENT_CHANNEL_ID = '1492185059325644860';
        ...
        if (interaction.commandName === 'info') {
            try {
                ...
                const INFO_CHANNEL_ID = '1481738998689960046';
        ...
        if (interaction.commandName === 'payes') {
            try {
                ...
                const ROLE_AMT = '1481739117967704186'; // 30%
                const ROLE_M = '1481739112049414335';  // 35%
                const ROLE_ME = '1481739106944811180'; // 40%
                const ROLE_CA = '1481739094412234773'; // 45%
    ...
    if (interaction.customId.startsWith('accept_')) {
        ...
        try {
            ...
            const member = await interaction.guild.members.fetch(userId);
            const INTERVIEW_WAITING_ROLE_ID = '1481739134220501002';
